...
const TwoTwoBook = ({ each_book, displayBooks, setDisplayBooks }) => {
    ...
    const handleDelete = each_book => {
        ...
        if (agree) {
            ...
            fetch(`https://ece-books-server.vercel.app/two_two/${each_book._id}`, {
                method: 'DELETE'
            })
                .then(res => res.json())
                .then(data => {
        ...
        <div className='book'>
            {
                isAdmin ?
                    <>
                        ...
                        <Link target="_blank" to={`/update_four/${each_book._id}`}>
