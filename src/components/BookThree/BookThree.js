...
const TwoOneBook = ({ each_book, displayBooks, setDisplayBooks }) => {
    ...
    const handleDelete = each_book => {
        ...
        if (agree) {
            ...
            fetch(`https://ece-books-server.vercel.app/two_one/${each_book._id}`, {
                method: 'DELETE'
            })
                .then(res => res.json())
                .then(data => {
        ...
        <div className='book'>
            {
                isAdmin ?
                    <>
                        ...
                        <Link target="_blank" to={`/update_three/${each_book._id}`}>
